...
### Cách chuẩn — site khách chạy theme `lacadev-client`

Nếu site khách dùng theme `lacadev-client` (hoặc theme con của nó), tracker **đã có sẵn ngay trong theme**, không cần cài thêm gì:

1. Trong màn hình sửa Project, mở hộp **📋 Lịch sử & Cảnh báo (Logs & Alerts)** — copy 2 giá trị **Endpoint URL** và **Secret Key** hiển thị sẵn ở đó (mục "Auto Activity Tracker").
2. Đăng nhập wp-admin site khách → **Laca Admin → 📡 Tracker** → dán 2 giá trị trên vào 2 ô tương ứng → Save.
3. Xong — tracker của theme (`LacaDevTrackerClient`) tự động chạy cron quét file + digest update pending, có sẵn cơ chế chống mất log khi mạng gián đoạn. Không cần đăng nhập hosting, không cần CHMOD file nào cả.

### Cách dự phòng — site khách KHÔNG chạy theme `lacadev-client`

Chỉ dùng cách này cho site khác (theme có sẵn của khách, WordPress thuần...). Trong hộp "Auto Activity Tracker" ở màn hình sửa Project, bấm **"🚀 Xem code PHP"** để mở popup SweetAlert2 hiển thị code (có nút **"📋 Copy Code"**), hoặc **"⬇️ Tải MU-plugin"** để tải file `.php` (tự gen, không lưu rác lên hosting LacaDev CMS). Code này bao gồm:
...
Đăng nhập vào hosting của web khách hàng, ở mục `wp-content/mu-plugins/` hãy ném đoạn mã hoặc upload file php bạn vừa tải về qua (nếu chưa có thì tự tạo tên tự do vd: `l-tracker.php`). Phân quyền file (CHMOD) thành `0444`.

⚠️ **Không dùng cách dự phòng này cho site đang chạy theme `lacadev-client`** — sẽ tạo ra 2 hệ tracker chạy song song, không đồng bộ với nhau (đúng lỗi đã từng xảy ra và được dọn ở phía theme client).
...
### Tự kiểm tra khi có gì bất thường

- Nếu Cronjob daily bị trễ quá 3 ngày (thường do site ít traffic — WordPress pseudo-cron chỉ chạy khi có người vào site), 1 cảnh báo màu vàng sẽ tự hiện ở đầu trang wp-admin, kèm hướng dẫn cài cron hệ thống thật (xem `doc/TRACKER_HUB_CLIENT_SYNC.md`).
- Nếu 1 kênh thông báo (Email/Zalo/Telegram/Slack) đang bật mà lần gửi gần nhất thất bại (vd token Zalo hết hạn), 1 cảnh báo màu đỏ sẽ tự hiện ở đầu trang, kèm lý do lỗi. Trang Dashboard chính cũng có ô "Kênh thông báo" tóm tắt tình trạng.

---

## 5. Client Portal — Cổng thông tin cho khách hàng

Mỗi project có 1 link Client Portal riêng (public, không cần khách đăng nhập WP) để gửi cho khách xem tiến độ mà không cần vào wp-admin.

- **Tạo link**: Tạo 1 Page mới, chọn Template **"Client Portal"**, publish. Link portal của từng project được hiển thị sẵn trong meta box sửa Project.
- **Khách xem được**: trạng thái dự án, % tiến độ, checklist công việc, nhật ký cập nhật/bảo trì (đã lọc — không hiện log bảo mật/sửa file), mốc thời gian (ngày bắt đầu/bàn giao), cảnh báo đang mở (trừ cảnh báo bảo mật), và **tình trạng thanh toán** (Tổng giá trị / Đã thanh toán / Còn lại — không hiện ghi chú tài chính nội bộ hay hoá đơn).
- **Khách gửi được**: yêu cầu hỗ trợ/báo lỗi/nội dung mới ngay trên portal (kèm ảnh đính kèm), không cần email/Zalo qua lại.
- Xác thực qua `_tracker_secret_key` của project (hoặc alias dễ nhớ `_portal_alias` nếu đã đặt) — không dùng tài khoản WP nên tuyệt đối không gửi kèm link portal cho người ngoài dự án.

---

## 6. Sổ tay File Logic (Files Reference)
...
- `app/src/Models/ProjectAlert.php` | `ProjectLog.php`
  - Hai Models quản lý việc Thêm/Xóa/Resolved các Logs và Alerts, thực thi tương tác qua DB tự sinh (custom db logic).

- `app/src/Features/ProjectManagement/ProjectPaymentService.php`
  - Tự tính `payment_status` (pending/partial/paid) từ `price_build` + `payment_history` mỗi khi lưu project. `readBuildPrice()`/`readTotalPaid()` là `public` để `ClientPortalEndpoint` tái dùng cho mục thanh toán trên Client Portal.
...
- `app/src/Settings/LacaTools/ProjectTrackerGenerator.php`
  - Sinh mã mu-plugin (template PHP nằm inline trong chính file này, không phải file `.txt` riêng) cho **cách dự phòng** (site khách không chạy theme `lacadev-client`) — bind Endpoint/Secret Key vào code rồi xuất qua popup hoặc Blob Download. Có sẵn cảnh báo ngay trong code sinh ra: không dùng cho site đang chạy `lacadev-client`.

- `app/src/Settings/LacaTools/TrackerEndpointHandler.php`
  - Rest API (`/laca/v1/tracker/log`) hứng log/heartbeat từ theme `lacadev-client` (đường chuẩn hiện tại) gửi về LacaDev CMS.

- `app/src/Features/ProjectManagement/Api/ClientWebhook.php`
  - Rest API (`/laca/v1/client-report`) hứng báo cáo từ site khách không chạy theme `lacadev-client` (đường dự phòng, dùng HMAC signature).

- `app/src/Settings/LacaTools/ClientPortalEndpoint.php`
  - Rest API phục vụ Client Portal (mục 5) — trả dữ liệu đã lọc cho khách xem, nhận yêu cầu hỗ trợ từ khách.
