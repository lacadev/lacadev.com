...
export const initAboutLacaHero = () => {
    ...
    if (!imgContainer || !content) return;
    ...
    gsap.set(imgContainer, { maxWidth: initialMaxWidth, borderRadius: '2rem' });
    gsap.set(content, { opacity: 0, y: 30 });
    if (parallaxBg) gsap.set(parallaxBg, { scale: 1.1 });
    ...
    tl.fromTo(imgContainer, 
        ...
        { maxWidth: fullWidth, borderRadius: '0rem', ease: 'none', force3D: true },
        0
    );

    // 2. Zoom out ảnh nền
    if (parallaxBg) {
        tl.fromTo(parallaxBg,
            { scale: 1.1 },
